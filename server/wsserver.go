...
import (
	...
	"github.com/ariadata/go-vpn/common/cipher"
	"github.com/ariadata/go-vpn/common/config"
	"github.com/ariadata/go-vpn/common/netutil"
	"github.com/ariadata/go-vpn/register"
	"github.com/ariadata/go-vpn/tun"
...
func StartWSServer(config config.Config) {
	...
	go tunToWs(iface, c)
	log.Printf("go-vpn ws server started on %v,CIDR is %v", config.LocalAddr, config.CIDR)
